      ...
      d_cycle_counter(0)
{
    ...
    // Guard time: 5ms or half the look-through window
    ...
    for (int i = 0; i <= d_fft_size; i++) {
        ...
        if (i < d_fft_size && db > d_reactive_threshold_db) {
            ...
            if (in_island) {
                ...
                if (d_sticky_denial) {
                    bool exists = false;
                    for (auto& existing : d_tracked_targets) {
                        if (std::abs(existing.center_freq - cf) < 15000.0) { 
                            exists = true; 
                            if (bw > existing.bandwidth) existing.bandwidth = bw;
...
// Setters
void interdictor_cpp_impl::set_technique(const std::string& technique) { d_technique = technique; }
void interdictor_cpp_impl::set_sample_rate_hz(double sample_rate_hz) { 
    d_sample_rate_hz = sample_rate_hz; 
    d_look_samples = static_cast<uint64_t>(d_look_through_ms * d_sample_rate_hz / 1000.0); 
    d_jam_samples = static_cast<uint64_t>(d_jam_cycle_ms * d_sample_rate_hz / 1000.0); 
    double guard_ms = std::min(5.0, d_look_through_ms / 2.0);
...
void interdictor_cpp_impl::set_look_through_ms(double ms) { 
    d_look_through_ms = ms; 
    d_look_samples = static_cast<uint64_t>(ms * d_sample_rate_hz / 1000.0); 
    double guard_ms = std::min(5.0, d_look_through_ms / 2.0);
    d_guard_samples = static_cast<uint64_t>(guard_ms * d_sample_rate_hz / 1000.0);
}
...
void interdictor_cpp_impl::set_targets(const std::vector<interdictor_cpp::Target>& targets) { d_tracked_targets = targets; }
                               ...
                               gr_vector_void_star &output_items)
{
    ...
    for (int i = 0; i < noutput_items; i++) {
        ...
        if (d_is_looking) {
            ...
            if (d_cycle_counter >= d_jam_samples) { 
                d_is_looking = true; d_cycle_counter = 0; d_fft_ptr = 0;
            }
        }
    }
    ...
    const double fs_inv = 1.0 / d_sample_rate_hz;
    ...
    if (d_output_mode == "Auto-Surgical") {
        ...
        for (auto& target : d_tracked_targets) {
            ...
            for (int i = 0; i < noutput_items; i++) {
                ...
                float frac = static_cast<float>(virtual_idx - idx_low);
                
                std::complex<float> base_sample = d_base_waveform[idx_low] * (1.0f - frac) + d_base_waveform[idx_high] * frac;
                
                // Optimized Phase Accumulator (prevents drift over time)
                float cos_val = cosf(static_cast<float>(target.phase_acc));
                float sin_val = sinf(static_cast<float>(target.phase_acc));
                out[i] += (base_sample * std::complex<float>(cos_val, sin_val)) * (scale * gate[i]);
        ...
        const double phase_inc = two_pi * freq_offset * fs_inv;
        // We use d_total_samples_processed for manual phase because it's usually short-term
        // but for safety, let's add a static phase accumulator for the manual mode too
        static double manual_phase_acc = 0;
        
        for (int i = 0; i < noutput_items; i++) {
            std::complex<float> base_sample = d_base_waveform[d_waveform_idx];
            d_waveform_idx = (d_waveform_idx + 1) % wf_len;
            
            float cos_val = cosf(static_cast<float>(manual_phase_acc));
            float sin_val = sinf(static_cast<float>(manual_phase_acc));
            out[i] = base_sample * std::complex<float>(cos_val, sin_val) * gate[i];
            
            manual_phase_acc = fmod(manual_phase_acc + phase_inc, two_pi);
