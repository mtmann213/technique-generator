...
class techniquepdu(gr.sync_block):
    ...
    def _generate_base(self):
        try:
            actual_tech = (
                self.warhead_technique if self.technique == 'Reactive Jammer' else self.technique
            )
            ...
            if 'songName' in params:
                params['songName'] = self.song_name
    ...
    def work(self, input_items, output_items):
        ...
        if self._need_regen:
            ...
            if current_active in streaming_types:
                self._setup_filter()
            else:
                self._base_samples = self._generate_base()
                self._base_ptr = 0
            ...
            if self._dwell_counter <= 0:
                ...
                if len(in0) >= fft_len:
                    ...
                    freq_axis = np.linspace(
                        -self.sample_rate_hz / 2, self.sample_rate_hz / 2, fft_len
                    )
                    for _ in range(self.num_targets):
                        peak_idx = np.argmax(temp_psd)
                        peak_val = temp_psd[peak_idx]
                        if peak_val > self.reactive_threshold_db:
                            target_f = freq_axis[peak_idx]
                            new_targets.append(target_f)
                            if self.adaptive_bw and _ == 0:
                                thresh_10db = peak_val - 10
                                left_idx = peak_idx
                                while left_idx > 0 and temp_psd[left_idx] > thresh_10db:
                                    left_idx -= 1
                                right_idx = peak_idx
                                while right_idx < fft_len - 1 and temp_psd[right_idx] > thresh_10db:
                                    right_idx += 1
                                measured_bw = (right_idx - left_idx) * (
                                    self.sample_rate_hz / fft_len
                                )
                                if abs(measured_bw - self.bandwidth_hz) > 5000:
                                    self.bandwidth_hz = max(
                                        10000, min(measured_bw, self.sample_rate_hz / 2)
                                    )
                                    self._setup_filter()
                            start = max(0, peak_idx - 20)
                            end = min(fft_len, peak_idx + 20)
                            temp_psd[start:end] = -150
                        else:
                            break
                    if new_targets:
                        self._target_freqs[: len(new_targets)] = new_targets
                        self._dwell_counter = int(
                            self.reactive_dwell_ms * self.sample_rate_hz / 1000.0
                        )
                        self._sabotage_counter = int(
                            self.sabotage_duration_ms * self.sample_rate_hz / 1000.0
                        )
                        self._drift_time = 0.0
                        self._stutter_timer = 0.0
                        # Randomize next clean count if enabled
                        if self.stutter_randomize:
                            self._current_cycle_clean = random.randint(
                                1, max(1, self.stutter_clean_count)
                            )
                        else:
                            self._current_cycle_clean = self.stutter_clean_count
        ...
        if self.manual_mode:
            self._target_freqs[0] = self.manual_freq
            self._dwell_counter = n + 100
        ...
        if is_reactive and self.preamble_sabotage:
            ...
            if rel_idx < self._current_cycle_clean:
                gate_open = False

        if not gate_open:
            out[:] = 0
            self._dwell_counter -= n
            self._stutter_timer += n / self.sample_rate_hz
            return n
        ...
        if use_realtime_noise:
            ...
            source_samples, self._filter_zi = signal.lfilter(
                self._filter_coeffs, 1.0, white, zi=self._filter_zi
            )
        else:
            s_len = len(self._base_samples)
            indices = (np.arange(n) + self._base_ptr) % s_len
            source_samples = self._base_samples[indices]
            self._base_ptr = (self._base_ptr + n) % s_len
        ...
        if is_reactive and not self.manual_mode:
            active_target_count = min(active_target_count, len(self._last_report_freqs))

        for tid in range(max(1, active_target_count)):
            f_target = self._target_freqs[tid]
            if is_swept:
                f_span = self.sweep_hz
                rate = (
                    self.sweep_rate_hz_s
                    if self.sweep_rate_hz_s > 0
                    else (f_span / self.technique_length_seconds)
                )
                period = f_span / rate if rate > 0 else 1.0
                f_inst = (
                    (2 * f_span / period) * np.abs((t_output % period) - period / 2) - (f_span / 2)
                    if self.sweep_type == 'triangle'
                    else (f_span / period) * (t_output % period) - (f_span / 2)
                )
                phases = (
                    2 * np.pi * np.cumsum(f_inst + f_target + freq_drift) / fs + self._phase[tid]
                )
                hydra_out += source_samples * np.exp(1j * phases)
                self._phase[tid] = phases[-1] % (2 * np.pi)
            else:
                phases = (
                    2 * np.pi * (f_target * t_drift + 0.5 * self.clock_pull_drift_hz_s * t_drift**2)
                    + self._phase[tid]
                )
                hydra_out += source_samples * np.exp(1j * phases)

        if active_target_count > 1:
            hydra_out /= np.sqrt(active_target_count)
        out[:] = hydra_out
    ...
    def handle_command(self, msg):
        if not self.enable_command_port:
            return
        try:
            ...
            for i in range(pmt.length(keys)):
                k_p = pmt.vector_ref(keys, i)
                k = pmt.symbol_to_string(k_p)
                v_p = pmt.dict_ref(msg, k_p, pmt.PMT_NIL)
                s = f"set_{k}"
                if hasattr(self, s):
                    if pmt.is_bool(v_p):
                        val = pmt.to_bool(v_p)
                    elif pmt.is_real(v_p) or pmt.is_integer(v_p):
                        val = pmt.to_double(v_p)
                    else:
                        val = pmt.symbol_to_string(v_p)
                    getattr(self, s)(val)
            self._need_regen = True
        except Exception as e:
            print(f"Error: {e}")
    ...
    def handle_trigger(self, msg):
        samples = self._generate_base()
        meta = pmt.make_dict()
        meta = pmt.dict_add(meta, pmt.intern("sample_rate"), pmt.from_double(self.sample_rate_hz))
        self.message_port_pub(
            pmt.intern("pdu"), pmt.cons(meta, pmt.init_c32vector(len(samples), samples))
        )

    def set_technique(self, v):
        self.technique = v
        self._need_regen = True

    def set_sample_rate_hz(self, v):
        self.sample_rate_hz = float(v)
        self._need_regen = True

    def set_bandwidth_hz(self, v):
        self.bandwidth_hz = float(v)
        self._need_regen = True

    def set_technique_length_seconds(self, v):
        self.technique_length_seconds = float(v)
        self._need_regen = True

    def set_interference_type(self, v):
        self.interference_type = str(v)
        self._need_regen = True

    def set_symbol_rate_hz(self, v):
        self.symbol_rate_hz = float(v)
        self._need_regen = True

    def set_rolloff(self, v):
        self.rolloff = float(v)
        self._need_regen = True

    def set_sweep_hz(self, v):
        self.sweep_hz = float(v)
        self._need_regen = True

    def set_sweep_type(self, v):
        self.sweep_type = str(v)
        self._need_regen = True

    def set_sweep_rate_hz_s(self, v):
        self.sweep_rate_hz_s = float(v)
        self._need_regen = True

    def set_technique_width_hz(self, v):
        self.technique_width_hz = float(v)
        self._need_regen = True

    def set_chunks(self, v):
        self.chunks = int(v)
        self._need_regen = True

    def set_frequencies_str(self, v):
        self.frequencies_str = str(v)
        self._need_regen = True

    def set_tones(self, v):
        self.tones = int(v)
        self._need_regen = True

    def set_sweep_range_hz(self, v):
        self.sweep_range_hz = float(v)
        self._need_regen = True

    def set_modulated_frequency(self, v):
        self.modulated_frequency = float(v)
        self._need_regen = True

    def set_song_name(self, v):
        self.song_name = str(v)
        self._need_regen = True

    def set_start_freq_hz(self, v):
        self.start_freq_hz = float(v)
        self._need_regen = True

    def set_end_freq_hz(self, v):
        self.end_freq_hz = float(v)
        self._need_regen = True

    def set_hop_frequencies_str(self, v):
        self.hop_frequencies_str = str(v)
        self._need_regen = True

    def set_hop_duration_seconds(self, v):
        self.hop_duration_seconds = float(v)
        self._need_regen = True
    ...
    def set_num_subcarriers(self, v):
        self.num_subcarriers = int(v)
        self._need_regen = True

    def set_cp_length(self, v):
        self.cp_length = int(v)
        self._need_regen = True

    def set_target_value(self, v):
        self.target_value = float(v)
        self._need_regen = True

    def set_normalization_type(self, v):
        self.normalization_type = str(v)
        self._need_regen = True

    def set_filter_type(self, v):
        self.filter_type = str(v)
        self._need_regen = True

    def set_reactive_threshold_db(self, v):
        self.reactive_threshold_db = float(v)

    def set_reactive_dwell_ms(self, v):
        self.reactive_dwell_ms = float(v)

    def set_warhead_technique(self, v):
        self.warhead_technique = str(v)
        self._need_regen = True

    def set_num_targets(self, v):
        self.num_targets = int(v)

    def set_manual_mode(self, v):
        self.manual_mode = bool(v)

    def set_manual_freq(self, v):
        self.manual_freq = float(v)

    def set_jamming_enabled(self, v):
        self.interdiction_enabled = bool(v)

    def set_adaptive_bw(self, v):
        self.adaptive_bw = bool(v)

    def set_preamble_sabotage(self, v):
        self.preamble_sabotage = bool(v)

    def set_sabotage_duration_ms(self, v):
        self.sabotage_duration_ms = float(v)

    def set_clock_pull_drift_hz_s(self, v):
        self.clock_pull_drift_hz_s = float(v)

    def set_stutter_enabled(self, v):
        self.stutter_enabled = bool(v)

    def set_stutter_clean_count(self, v):
        self.stutter_clean_count = int(v)

    def set_stutter_burst_count(self, v):
        self.stutter_burst_count = int(v)

    def set_stutter_randomize(self, v):
        self.stutter_randomize = bool(v)

    def set_frame_duration_ms(self, v):
        self.frame_duration_ms = float(v)

    def set_enable_command_port(self, v):
        self.enable_command_port = bool(v)

    def set_output_mode(self, v):
        self.output_mode = str(v)
        self._need_regen = True

    def set_predictive_tracking(self, v):
        self.predictive_tracking = bool(v)
