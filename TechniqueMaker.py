...
def setup_env():
    ...
    env["PYTHONPATH"] = os.pathsep.join(
        new_paths + ([existing_pythonpath] if existing_pythonpath else [])
    )
...
Examples:
  ...
  ./TechniqueMaker.py install      # Run the OOT installation script
    ...
    parser.add_argument(
        "mode", choices=["gui", "predator", "calibrate", "batch", "install"], help="What to launch"
    )
    parser.add_argument(
        "--args", nargs=argparse.REMAINDER, help="Additional arguments for the sub-command"
    )
