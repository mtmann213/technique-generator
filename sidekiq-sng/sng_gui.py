...
class SidekiqSngGui:
    ...
    def setup_ui(self):
        ...
        ttk.Button(header_frame, text="Probe Ports", command=self.probe_ports).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(
            header_frame, text="Test Port 0 (Blink)", command=lambda: self.test_port(0)
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(
            header_frame, text="Test Port 1 (Blink)", command=lambda: self.test_port(1)
        ).pack(side=tk.LEFT, padx=5)
        ...
        for i, (label, key, default) in enumerate(settings):
            ttk.Label(rf_frame, text=label).grid(
                row=i // 3, column=(i % 3) * 2, sticky=tk.W, padx=5
            )
            var = tk.StringVar(value=default)
            self.vars[key] = var
            ttk.Entry(rf_frame, textvariable=var, width=15).grid(
        ...
        self.console = tk.Text(
            console_frame, height=10, bg="black", fg="lightgreen", font=("monospace", 10)
        )
        ...
        ttk.Button(btn_frame, text="Generate to File", command=self.generate_to_file).pack(
            side=tk.LEFT, fill=tk.X, expand=True, padx=5
        )
        ttk.Button(btn_frame, text="Stop / Kill", command=self.stop_process).pack(
            side=tk.LEFT, fill=tk.X, expand=True, padx=5
        )
    ...
    def update_params(self, event=None):
        ...
        if tech == "fhss":
            ...
            params = [
                ("Phase Shift (deg):", "shift", "180"),
                ("Shift Rate (Hz):", "shift-rate", "1000"),
            ]
        elif tech == "fm-cosine":
        ...
        for i, (label, key, default) in enumerate(params):
            ttk.Label(self.param_frame, text=label).grid(
                row=i // 2, column=(i % 2) * 2, sticky=tk.W, padx=5
            )
            var = tk.StringVar(value=default)
            self.param_vars[key] = var
            ttk.Entry(self.param_frame, textvariable=var, width=20).grid(
                row=i // 2, column=(i % 2) * 2 + 1, padx=5, pady=2
            )
