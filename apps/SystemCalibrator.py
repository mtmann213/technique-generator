...
class TableWindow(Qt.QWidget):
    def __init__(self, f_list, p_targets, gain_matrix):
        ...
        self.table.setRowCount(len(f_list))
        self.table.setColumnCount(len(p_targets))
...
class TransmitBlock(gr.top_block):
        ...
        if technique == "Direct CW":
            ...
            try:
                ...
                self.sys_logger.warning(
                    "C++ interdictor block not found. Falling back to Python techniquepdu."
                )
    ...
    def set_technique(self, tech):
        if hasattr(self.src, "set_technique"):
            self.src.set_technique(tech)
...
class ReceiveBlock(gr.top_block):
    def __init__(self, serial="3457464", samp_rate=2e6, freq=915e6, gain=40):
        ...
        self.connect(self.source, self.snk)
...
class SystemCalibrator(Qt.QWidget):
    def __init__(self):
        ...
        self.sdr = None
        self.presets = {}
        self.preset_file = "config/calibrator_presets.json"
        ...
        preset_grid = Qt.QGridLayout()
        preset_box.setLayout(preset_grid)
        self.preset_combo = Qt.QComboBox()
        self.preset_combo.currentTextChanged.connect(self.load_selected_preset)
        preset_grid.addWidget(Qt.QLabel("Preset:"), 0, 0)
        preset_grid.addWidget(self.preset_combo, 0, 1)
        self.save_btn = Qt.QPushButton("Save New")
        self.save_btn.clicked.connect(self.save_current_preset)
        preset_grid.addWidget(self.save_btn, 1, 0)
        self.del_btn = Qt.QPushButton("Delete")
        self.del_btn.clicked.connect(self.delete_current_preset)
        preset_grid.addWidget(self.del_btn, 1, 1)
        self.scroll_layout.addWidget(preset_box)

        # Hardware Setup
        hw_box = Qt.QGroupBox("Hardware Configuration")
        hw_layout = Qt.QFormLayout()
        hw_box.setLayout(hw_layout)
        self.tx_serial = Qt.QLineEdit(
            self.config_manager.get("hardware", "tx_usrp_serial", "34573DD")
        )
        hw_layout.addRow("TX Serial:", self.tx_serial)
        self.rx_mode = Qt.QComboBox()
        self.rx_mode.addItems(
            ["USRP (UHD)", "Signal Hound (Soapy)", "Manual Entry (Spike)"]
        )
        hw_layout.addRow("Receiver Type:", self.rx_mode)
        self.rx_serial = Qt.QLineEdit(
            self.config_manager.get("hardware", "rx_usrp_serial", "3457464")
        )
        hw_layout.addRow("RX Serial:", self.rx_serial)
        self.rx_gain_input = Qt.QLineEdit(
            str(self.config_manager.get("rf_defaults", "rx_gain", 40))
        )
        hw_layout.addRow("RX USRP Gain (dB):", self.rx_gain_input)
        self.atten_ext = Qt.QLineEdit(
            str(self.config_manager.get("rf_defaults", "external_attenuation_db", 30))
        )
        hw_layout.addRow("Ext Atten (dB):", self.atten_ext)
        self.scroll_layout.addWidget(hw_box)

        # Sweep Configuration
        sw_box = Qt.QGroupBox("Sweep Configuration")
        sw_layout = Qt.QFormLayout()
        sw_box.setLayout(sw_layout)
        self.cal_mode = Qt.QComboBox()
        self.cal_mode.addItems(["Full Matrix (Gain/Freq)", "Technique Comparison"])
        sw_layout.addRow("Action:", self.cal_mode)
        self.tech_select = Qt.QComboBox()
        self.tech_select.addItems(
            ["Direct CW", "CW Tone (Pure)"]
            + list(BaseWaveforms.waveform_definitions.keys())
        )
        sw_layout.addRow("TX Template:", self.tech_select)
        self.f_start = Qt.QLineEdit("900e6")
        self.f_stop = Qt.QLineEdit("930e6")
        self.f_step = Qt.QLineEdit("5e6")
        sw_layout.addRow("Freq Start:", self.f_start)
        sw_layout.addRow("Freq Stop:", self.f_stop)
        sw_layout.addRow("Freq Step:", self.f_step)
        self.g_start = Qt.QLineEdit("30")
        self.g_stop = Qt.QLineEdit("70")
        self.g_step = Qt.QLineEdit("5")
        sw_layout.addRow("TX Gain Start:", self.g_start)
        sw_layout.addRow("TX Gain Stop:", self.g_stop)
        sw_layout.addRow("TX Gain Step:", self.g_step)
        self.scroll_layout.addWidget(sw_box)

        # Operational View Config
        ana_box = Qt.QGroupBox("Operational Table Settings")
        ana_layout = Qt.QFormLayout()
        ana_box.setLayout(ana_layout)
        self.p_start = Qt.QLineEdit("-20")
        self.p_stop = Qt.QLineEdit("30")
        self.p_step = Qt.QLineEdit("5")
        ana_layout.addRow("Target Min dBm:", self.p_start)
        ana_layout.addRow("Target Max dBm:", self.p_stop)
        ana_layout.addRow("Target Step dB:", self.p_step)
        self.view_select = Qt.QComboBox()
        self.view_select.addItems(["Measurement View", "Operational View"])
        self.view_select.currentIndexChanged.connect(self.update_plot)
        ...
        self.table_btn = Qt.QPushButton("OPEN DATA TABLE")
        self.table_btn.clicked.connect(self.show_table_window)
        data_layout.addWidget(self.table_btn)
        self.csv_btn = Qt.QPushButton("EXPORT CSV")
        ...
        self.start_btn.setStyleSheet(
            "background-color: #050; color: white; font-weight: bold; height: 40px;"
        )
        ...
        self.status_label.setStyleSheet(
            "font-family: monospace; background: black; color: #0F0; padding: 5px;"
        )
    ...
    def save_current_preset(self):
        ...
        if ok and name:
            self.presets[name] = {
                "tx_serial": self.tx_serial.text(),
                "rx_mode": self.rx_mode.currentText(),
                "rx_serial": self.rx_serial.text(),
                "f_start": self.f_start.text(),
                "f_stop": self.f_stop.text(),
                "f_step": self.f_step.text(),
                "g_start": self.g_start.text(),
                "g_stop": self.g_stop.text(),
                "g_step": self.g_step.text(),
                "atten": self.atten_ext.text(),
                "p_start": self.p_start.text(),
                "p_stop": self.p_stop.text(),
                "p_step": self.p_step.text(),
                "cal_mode": self.cal_mode.currentText(),
                "tech": self.tech_select.currentText(),
            }
    ...
    def delete_current_preset(self):
        ...
        if name in self.presets:
            ...
            json.dump(self.presets, open(self.preset_file, "w"), indent=4)
            self.load_presets_from_file()
    ...
    def load_selected_preset(self, name):
        if name in self.presets:
            ...
            self.f_start.setText(p.get("f_start", "900e6"))
            self.f_stop.setText(p.get("f_stop", "930e6"))
            self.f_step.setText(p.get("f_step", "5e6"))
            self.g_start.setText(p.get("g_start", "30"))
            self.g_stop.setText(p.get("g_stop", "70"))
            self.g_step.setText(p.get("g_step", "5"))
            self.p_start.setText(p.get("p_start", "-20"))
            self.p_stop.setText(p.get("p_stop", "30"))
            self.p_step.setText(p.get("p_step", "5"))
    ...
    def start_calibration(self):
        try:
            if self.cal_mode.currentText() == "Technique Comparison":
                self.sweep_techs = ["Direct CW", "CW Tone (Pure)"] + list(
                    BaseWaveforms.waveform_definitions.keys()
                )
                self.sweep_freqs = [
                    float(parse_scientific_notation(self.f_start.text()))
                ]
                ...
                fs, fe, fstep = (
                    float(parse_scientific_notation(self.f_start.text())),
                    float(parse_scientific_notation(self.f_stop.text())),
                    float(parse_scientific_notation(self.f_step.text())),
                )
                num_f = int(round((fe - fs) / fstep)) + 1
                self.sweep_freqs = np.linspace(fs, fe, num_f)
                gs, ge, gstep = (
                    float(self.g_start.text()),
                    float(self.g_stop.text()),
                    float(self.g_step.text()),
                )
                num_g = int(round((ge - gs) / gstep)) + 1
                self.sweep_gains = np.linspace(gs, ge, num_g)

            self.total_steps = (
                len(self.sweep_freqs) * len(self.sweep_gains)
                if self.cal_mode.currentText() != "Technique Comparison"
                else len(self.sweep_techs)
            )
            self.current_step = 0
            self.results = {}
            self.tech_deltas = {}
            ...
            self.log(
                f"Starting TX: USRP {self.tx_serial.text()} ({self.sweep_techs[0]})"
            )
            self.tb = TransmitBlock(
                serial=self.tx_serial.text(), technique=self.sweep_techs[0]
            )
            self.tb.start()

            mode = self.rx_mode.currentText()
            if mode == "USRP (UHD)":
                self.rb = ReceiveBlock(
                    serial=self.rx_serial.text(), gain=float(self.rx_gain_input.text())
                )
                self.rb.start()
    ...
    def stop_calibration(self):
        ...
        self.start_btn.setStyleSheet("background-color: #050; color: white;")
        try:
            if self.tb:
                self.tb.stop()
                self.tb.wait()
            ...
            if self.rb:
                ...
                self.rb = None
            if self.sdr:
                self.sdr.deactivateStream(None)
                self.sdr.closeStream(None)
                self.sdr = None
        except Exception:
            pass
    ...
    def run_auto_step(self):
        ...
        try:
            if self.cal_mode.currentText() == "Technique Comparison":
                if self.current_step >= len(self.sweep_techs):
                    self.finish_calibration()
                    return
                tech = self.sweep_techs[self.current_step]
                freq = self.sweep_freqs[0]
                gain = self.sweep_gains[0]
                self.tb.stop()
                self.tb.wait()
                self.tb = TransmitBlock(
                    serial=self.tx_serial.text(), technique=tech, freq=freq, gain=gain
                )
                self.tb.start()
            else:
                f_idx = self.current_step // len(self.sweep_gains)
                g_idx = self.current_step % len(self.sweep_gains)
                if f_idx >= len(self.sweep_freqs):
                    self.finish_calibration()
                    return
                freq = self.sweep_freqs[f_idx]
                gain = self.sweep_gains[g_idx]
                ...
                if self.rb:
                    ...
                    if samples is not None:
                        measurements.append(
                            10
                            * np.log10(
                                np.abs(np.fft.fft(samples)) ** 2 / len(samples) + 1e-12
                            ).max()
                        )
                elif self.sdr:
                    ...
                    self.sdr.readStream(None, [buff], len(buff))
                    measurements.append(
                        10
                        * np.log10(
                            np.abs(np.fft.fft(buff)) ** 2 / len(buff) + 1e-12
                        ).max()
                    )

            actual_p = (
                np.mean(measurements) + float(self.atten_ext.text())
                if measurements
                else -100
            )
            ...
            if self.cal_mode.currentText() == "Technique Comparison":
                ...
                self.log(
                    f"Analyzed {self.sweep_techs[self.current_step]}: {actual_p:.1f} dBm"
                )
            else:
    ...
    def get_operational_data(self):
        ...
        if not f_list:
            return [], [], np.array([])
        ps, pe, pstep = (
            float(self.p_start.text()),
            float(self.p_stop.text()),
            float(self.p_step.text()),
        )
        num_p = int(round((pe - ps) / pstep)) + 1
        p_targets = np.linspace(ps, pe, num_p)
        gain_matrix = np.zeros((len(f_list), len(p_targets)))
        for i, f in enumerate(f_list):
            gm = sorted(self.results[f].keys())
            pm = [self.results[f][g] for g in gm]
            cg = []
            cp = []
            lp = -200
            ...
            if len(cp) < 2:
                ...
                gain_matrix[i, :] = np.clip(
                    interp1d(cp, cg, fill_value="extrapolate")(p_targets), 0, 90
                )
    ...
    def update_plot(self):
        ...
        if self.cal_mode.currentText() == "Technique Comparison":
            if self.tech_deltas:
                ...
                base = self.tech_deltas.get("Direct CW", vals[0])
                offsets = [v - base for v in vals]
                ax.barh(names, offsets, color="skyblue")
                ax.set_title("Template Power Offsets (PAPR)")
        elif self.results:
            if "Operational" in self.view_select.currentText():
                ...
                im = ax.imshow(
                    data,
                    aspect="auto",
                    extent=[g_sweep[0], g_sweep[-1], f_list[0] / 1e6, f_list[-1] / 1e6],
                    origin="lower",
                    cmap="plasma",
                )
        self.canvas.draw()
    ...
    def show_table_window(self):
        if self.results:
            f_list, p_targets, gain_matrix = self.get_operational_data()
            self.table_win = TableWindow(f_list, p_targets, gain_matrix)
            self.table_win.show()
    ...
    def export_csv(self):
        if self.results:
            ...
            path, _ = Qt.QFileDialog.getSaveFileName(
                self, "Export", "cal_table.csv", "CSV (*.csv)"
            )
            if path:
                with open(path, "w", newline="") as f:
                    w = csv.writer(f)
                    w.writerow(["Freq (MHz) \\ dBm"] + [f"{p}" for p in p_targets])
                    for i, freq in enumerate(f_list):
                        w.writerow([f"{freq / 1e6:.2f}"] + list(gain_matrix[i, :]))
    ...
    def finish_calibration(self):
        ...
        with open("config/calibration_matrix.json", "w") as f:
