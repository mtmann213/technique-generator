...
class WaveformApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.waveform_combobox = ttk.Combobox(
            wf_frame, values=list(self.waveform_definitions.keys()), state="readonly"
        )
        ...
        self.format_combobox = ttk.Combobox(
            fmt_frame, values=list(self.file_format_definitions.keys()), state="readonly"
        )
        ...
        self.save_btn = ttk.Button(
            btn_frame, text="Generate and Save File", command=self.generate_and_save
        )
        self.save_btn.pack(side="top", fill="x", pady=2)

        self.status_label = ttk.Label(
            self.scrollable_frame, text="Ready", relief="sunken", anchor="w"
        )
        ...
        for p in wf_info["params"]:
            ...
            if p['type'] == "options":
                ...
                if p['name'] == 'sample_rate_hz':
                    var.set("1000000")
                elif p['name'] == 'technique_length_seconds':
                    var.set("0.1")
                elif p['name'] == 'target_value':
                    var.set("1.0")
    ...
    def update_preview(self):
        ...
        try:
            ...
            params = {
                name: infer_type_from_string(var.get()) for name, var in self.param_vars.items()
            }
