"""
Predator Console — Refactored Edition (Phase 2)

The original 834-line monolith has been decomposed:
  apps/gui/       — Theme engine, styles, widget helpers
  apps/hardware/  — USRP discovery, connection handling
  apps/session/   — Presets, calibration management
  apps/engine/    — GNU Radio flowgraph (coming next)

This file is now the orchestrator — it wires the modules together.
The core logic is unchanged; imports are cleaner.
"""
...
from session.calibration import CalibrationManager
from hardware.usrp_discovery import scan_usrps
...
class PredatorJammer(gr.top_block, Qt.QWidget):
    def __init__(self):
        ...
        # --- 1. GLOBAL HEADER ---
        self._build_header(default_serial)
        ...
        self.timer.timeout.connect(self.check_detections)
        self.timer.start(100)
        self.sim_timer = QtCore.QTimer()
        self.sim_timer.timeout.connect(self.on_sim_hop)

    # ============================================================
    # Layout builders (extracted from __init__)
    # ============================================================
    ...
    def _build_header(self, default_serial):
        ...
        tuning_box.setStyleSheet(f"QGroupBox {{ {group_style(self.theme)} }}")
        ...
        self.freq_input.setStyleSheet(input_style(self.theme))
        ...
        self.samp_input.setStyleSheet(input_style(self.theme))
        ...
        self.apply_btn.setStyleSheet(apply_button_style())
        ...
        self.status_label.setStyleSheet(STATUS_OFFLINE)
    ...
    def _build_hardware_tab(self):
        ...
        hw_disc_box.setStyleSheet(f"QGroupBox {{ {group_style(self.theme)} }}")
        ...
        self.sh_serial_input.setStyleSheet(input_style(self.theme))
        ...
        # Secondary TX Group
        self._build_secondary_warhead_section(hw_layout)

        # RF Output
        rf_box = Qt.QGroupBox("Gain & RF Output")
        rf_box.setStyleSheet(f"QGroupBox {{ {group_style(self.theme)} }}")
        rf_grid = Qt.QFormLayout(rf_box)

        self.rx_gain_slider = Qt.QSlider(QtCore.Qt.Horizontal)
        self.rx_gain_slider.setRange(0, 76)
        self.rx_gain_slider.setValue(40)
        self.rx_gain_slider.valueChanged.connect(self.on_rx_gain_change)
        rf_grid.addRow("RX Gain", self.rx_gain_slider)

        self.tx_gain_slider = Qt.QSlider(QtCore.Qt.Horizontal)
        self.tx_gain_slider.setRange(0, 89)
        self.tx_gain_slider.setValue(50)
        self.tx_gain_slider.valueChanged.connect(self.on_tx_gain_change)
        rf_grid.addRow("TX Gain", self.tx_gain_slider)

        self.tx_level_input = Qt.QLineEdit("-10.0")
        self.tx_level_input.setFixedWidth(80)
        self.tx_level_input.editingFinished.connect(self.on_tx_level_change)
        self.tx_level_input.setStyleSheet(input_style(self.theme))
        self.tx_level_input.setHidden(True)
        rf_grid.addRow("TX Level (dBm)", self.tx_level_input)

        self.cal_label = Qt.QLabel("Est. Output: --- dBm")
        self.cal_label.setStyleSheet(f"color: {c['accent_cyan']}; font-weight: bold;")
        rf_grid.addRow(self.cal_label)

        self.fire_btn = Qt.QPushButton("DISABLE TRANSMIT")
        self.fire_btn.setCheckable(True)
        self.fire_btn.setStyleSheet(safe_button())
        self.fire_btn.toggled.connect(self.on_fire_toggle)
        rf_grid.addRow(self.fire_btn)

        hw_layout.addWidget(rf_box)

        # Session & Records
        pre_box = Qt.QGroupBox("Session & Records")
        pre_box.setStyleSheet(f"QGroupBox {{ {group_style(self.theme)} }}")
        ...
        pre_grid.addWidget(Qt.QLabel("Preset:"), 0, 0)
        pre_grid.addWidget(self.preset_combo, 0, 1)
        self.save_btn = Qt.QPushButton("Save Preset")
        self.save_btn.clicked.connect(self.save_current_preset)
        pre_grid.addWidget(self.save_btn, 1, 0)
        self.record_btn = Qt.QPushButton("LOG SIGMF")
        self.record_btn.setCheckable(True)
        self.record_btn.toggled.connect(self.on_record_toggle)
        pre_grid.addWidget(self.record_btn, 1, 1)
        hw_layout.addWidget(pre_box)
        hw_layout.addStretch()

    def _build_secondary_warhead_section(self, parent_layout):
        self.sec_box = Qt.QGroupBox("Secondary Warhead Control")
        self.sec_box.setStyleSheet(f"QGroupBox {{ {group_style(self.theme)} }}")
        ...
        self.secondary_serial_combo = Qt.QComboBox()
        self.secondary_serial_combo.setEditable(True)
        self.secondary_serial_combo.addItem(self.secondary_serial)
        ...
        self.tx2_freq_input.setStyleSheet(highlight_input(self.theme))
        ...
        self.tx2_gain_slider = Qt.QSlider(QtCore.Qt.Horizontal)
        self.tx2_gain_slider.setRange(0, 89)
        self.tx2_gain_slider.setValue(50)
        self.tx2_gain_slider.valueChanged.connect(self.on_tx2_gain_change)
        ...
        parent_layout.addWidget(self.sec_box)

    def _build_interdiction_tab(self):
        int_tab = Qt.QWidget()
        int_layout = Qt.QVBoxLayout(int_tab)
        self.tabs.addTab(int_tab, "Interdiction")

        mode_box = Qt.QGroupBox("Target Selection")
        mode_box.setStyleSheet(f"QGroupBox {{ {group_style(self.theme)} }}")
        mode_layout = Qt.QVBoxLayout(mode_box)
        self.auto_radio = Qt.QRadioButton("Automatic Dynamic Tracking")
        self.auto_radio.setChecked(True)
        self.auto_radio.toggled.connect(self.on_mode_change)
        mode_layout.addWidget(self.auto_radio)
        self.manual_radio = Qt.QRadioButton("Manual Frequency Offset")
        self.manual_radio.toggled.connect(self.on_mode_change)
        mode_layout.addWidget(self.manual_radio)
        int_layout.addWidget(mode_box)

        hydra_box = Qt.QGroupBox("Hydra Auto-Surgical Engine")
        hydra_box.setStyleSheet(f"QGroupBox {{ {group_style(self.theme)} }}")
        hydra_grid = Qt.QFormLayout(hydra_box)
        self.hydra_cb = Qt.QCheckBox("Enable Autonomous Comb")
        self.hydra_cb.toggled.connect(self.on_hydra_toggle)
        hydra_grid.addRow(self.hydra_cb)
        self.predictive_cb = Qt.QCheckBox("Enable Predictive Tracking (PRNG Cracker)")
        self.predictive_cb.toggled.connect(self.on_predictive_toggle)
        hydra_grid.addRow(self.predictive_cb)
        self.adapt_cb = Qt.QCheckBox("Adaptive Bandwidth Sculpting")
        self.adapt_cb.toggled.connect(self.on_adapt_toggle)
        hydra_grid.addRow(self.adapt_cb)

        self.targets_slider = Qt.QSlider(QtCore.Qt.Horizontal)
        self.targets_slider.setRange(1, 16)
        self.targets_slider.setValue(1)
        self.targets_slider.valueChanged.connect(self.on_targets_change)
        self.targets_label = Qt.QLabel("Max Targets: 1")
        hydra_grid.addRow(self.targets_label, self.targets_slider)

        self.thresh_slider = Qt.QSlider(QtCore.Qt.Horizontal)
        self.thresh_slider.setRange(-120, 0)
        self.thresh_slider.setValue(-45)
        self.thresh_slider.valueChanged.connect(self.on_threshold_change)
        self.thresh_label = Qt.QLabel("Threshold: -45 dB")
        hydra_grid.addRow(self.thresh_label, self.thresh_slider)
        int_layout.addWidget(hydra_box)

        sticky_box = Qt.QGroupBox("Sticky Channel Denial")
        sticky_box.setStyleSheet(f"QGroupBox {{ {group_style(self.theme)} }}")
        sticky_grid = Qt.QFormLayout(sticky_box)
        self.sticky_cb = Qt.QCheckBox("Enable Persistent Trap")
        self.sticky_cb.toggled.connect(self.on_sticky_toggle)
        sticky_grid.addRow(self.sticky_cb)
        self.look_input = Qt.QLineEdit("10.0")
        self.look_input.setStyleSheet(input_style(self.theme))
        self.look_input.editingFinished.connect(self.on_look_change)
        sticky_grid.addRow("Look-thru (ms):", self.look_input)
        self.cycle_input = Qt.QLineEdit("90.0")
        self.cycle_input.setStyleSheet(input_style(self.theme))
        self.cycle_input.editingFinished.connect(self.on_jam_cycle_change)
        sticky_grid.addRow("Jam Cycle (ms):", self.cycle_input)
        self.reset_denial_btn = Qt.QPushButton("FLUSH DENIAL GRID")
        self.reset_denial_btn.clicked.connect(self.on_reset_denial)
        self.reset_denial_btn.setStyleSheet(danger_button())
        sticky_grid.addRow(self.reset_denial_btn)
        int_layout.addWidget(sticky_box)

        self.manual_box = Qt.QGroupBox("Manual Fine-Tuning")
        self.manual_box.setStyleSheet(f"QGroupBox {{ {group_style(self.theme)} }}")
        self.man_layout = Qt.QVBoxLayout(self.manual_box)
        self.manual_slider = Qt.QSlider(QtCore.Qt.Horizontal)
        self.manual_slider.setRange(-1000000, 1000000)
        self.manual_slider.setEnabled(False)
        self.manual_slider.valueChanged.connect(self.on_manual_freq_change)
        self.man_layout.addWidget(self.manual_slider)
        self.manual_label = Qt.QLabel("Offset: 0.0 kHz")
        self.manual_label.setAlignment(QtCore.Qt.AlignCenter)
        self.man_layout.addWidget(self.manual_label)
        int_layout.addWidget(self.manual_box)
        int_layout.addStretch()

    def _build_protocol_tab(self):
        prot_tab = Qt.QWidget()
        prot_layout = Qt.QVBoxLayout(prot_tab)
        self.tabs.addTab(prot_tab, "Protocol")

        adv_box = Qt.QGroupBox("Advanced Manipulations")
        adv_box.setStyleSheet(f"QGroupBox {{ {group_style(self.theme)} }}")
        adv_grid = Qt.QFormLayout(adv_box)
        self.sab_cb = Qt.QCheckBox("Preamble Sabotage (Invisible)")
        self.sab_cb.toggled.connect(self.on_sab_toggle)
        adv_grid.addRow(self.sab_cb)
        self.sab_input = Qt.QLineEdit(str(self.sabotage_duration))
        self.sab_input.setStyleSheet(input_style(self.theme))
        self.sab_input.editingFinished.connect(self.on_sab_duration_change)
        adv_grid.addRow("Duration (ms):", self.sab_input)
        self.stutter_cb = Qt.QCheckBox("Stability Frame Stutter")
        self.stutter_cb.toggled.connect(self.on_stutter_toggle)
        adv_grid.addRow(self.stutter_cb)
        self.stutter_clean_input = Qt.QLineEdit(str(self.stutter_clean))
        self.stutter_clean_input.setStyleSheet(input_style(self.theme))
        self.stutter_clean_input.editingFinished.connect(self.on_stutter_clean_change)
        adv_grid.addRow("Clean Count:", self.stutter_clean_input)
        self.stutter_burst_input = Qt.QLineEdit(str(self.stutter_burst))
        self.stutter_burst_input.setStyleSheet(input_style(self.theme))
        self.stutter_burst_input.editingFinished.connect(self.on_stutter_burst_change)
        adv_grid.addRow("Burst Count:", self.stutter_burst_input)
        self.frame_input = Qt.QLineEdit(str(self.frame_dur))
        self.frame_input.setStyleSheet(input_style(self.theme))
        self.frame_input.editingFinished.connect(self.on_frame_dur_change)
        adv_grid.addRow("Frame Dur (ms):", self.frame_input)
        self.pull_input = Qt.QLineEdit(str(self.clock_pull))
        self.pull_input.setStyleSheet(input_style(self.theme))
        self.pull_input.editingFinished.connect(self.on_pull_input_change)
        adv_grid.addRow("Clock-Pull (Hz/s):", self.pull_input)
        prot_layout.addWidget(adv_box)
    ...
    def _build_warhead_section(self):
        ...
        self.template_combo.addItems(list(BaseWaveforms.waveform_definitions.keys()))
        self.template_combo.currentTextChanged.connect(self.on_template_change)
        template_layout.addWidget(self.template_combo)

        self.param_group = Qt.QGroupBox("Technique Parameters")
        self.param_group.setStyleSheet(f"QGroupBox {{ {group_style(self.theme)} }}")
        self.param_layout = Qt.QFormLayout()
    ...
    def _build_waterfall(self):
        ...
        self.middle_split.addWidget(self.pyqt_widget, stretch=5)

    def _build_history_panel(self):
        self.history_panel = Qt.QVBoxLayout()
        self.history_list = Qt.QListWidget()
        self.history_list.setFixedWidth(250)
        self.history_list.setStyleSheet(log_style(self.theme))
        self.middle_split.addLayout(self.history_panel)
        self.history_panel.addWidget(Qt.QLabel("DYNAMIC TRACK LOG"))
        self.history_panel.addWidget(self.history_list)
        clear_hist = Qt.QPushButton("Clear")
        clear_hist.clicked.connect(self.history_list.clear)
        self.history_panel.addWidget(clear_hist)

    # ============================================================
    # Handlers & logic (preserved from original, with style updates)
    # ============================================================
    ...
    def on_sim_hop(self):
        if not self.sim_src:
            return
        if not hasattr(self, "_sim_lcg_state"):
    ...
    def on_scan_clicked(self):
        ...
        if not devices:
            ...
            return
        for dev in devices:
            self.serial_combo.addItem(dev["display"])
            self.secondary_serial_combo.addItem(dev["display"])
        self.sys_logger.info(f"Found {len(devices)} device(s).")
    ...
    def on_connect_toggled(self, checked):
        if checked:
            ...
            try:
                ...
                self.connect_btn.setStyleSheet(danger_button())
                self.status_label.setText("CONNECTED")
                self.status_label.setStyleSheet(STATUS_ONLINE)
    ...
    def init_blocks(self):
        if self.hardware_connected and self.serial:
            ...
            self.hw_source.set_gain(self.rx_gain, 0)
        else:
    ...
    def on_freq_change(self):
        try:
            ...
            if not self.sim_mode and hasattr(self, "hw_source"):
                if hasattr(self.hw_source, "set_center_freq"):
                    self.hw_source.set_center_freq(self.center_freq, 0)
            if hasattr(self, "sink") and self.sink:
                if hasattr(self.sink, "set_center_freq"):
                    self.sink.set_center_freq(self.center_freq, 0)
    ...
    def on_sink_type_change(self, text):
        if "Sidekiq" in text:
            self.tx_sink_type = "Sidekiq"
        elif "SoapySDR" in text:
            self.tx_sink_type = "SoapySDR"
        else:
            self.tx_sink_type = "UHD"
    ...
    def on_tx_level_change(self):
        try:
            ...
            if val > 10.0:
                self.sys_logger.warning(f"TX Level {val} exceeds 10.0 dBm. Clipping.")
                val = 10.0
                self.tx_level_input.setText("10.0")
            elif val < -120.0:
                self.sys_logger.warning(f"TX Level {val} below -120.0 dBm. Clipping.")
                val = -120.0
                self.tx_level_input.setText("-120.0")
            self.tx_level = val
    ...
    def on_sync_toggle(self, checked):
        ...
        if checked:
            ...
            self.tx2_template_combo.setCurrentText(self.template)
            self.on_tx2_freq_change()
    ...
    def check_detections(self):
        ...
        if not self.interdiction_enabled:
            ...
            self.status_label.setStyleSheet(STATUS_SILENT)
            return
    ...
    def on_rx_gain_change(self, value):
        self.rx_gain = value
        if not self.sim_mode and hasattr(self, "hw_source"):
            self.hw_source.set_gain(value, 0)
    ...
    def on_tx_gain_change(self, value):
        ...
        if self.sync_tx1:
            self.tx2_gain = self.tx_gain
            self.tx2_gain_slider.setValue(self.tx2_gain)
        if hasattr(self, "sink") and self.sink and self.tx_sink_type == "UHD":
    ...
    def load_selected_preset(self, name):
        ...
        if not p:
            return
        self.preset_manager.apply_to(self, name)
        # Update UI elements to match new state
        self.rx_gain_slider.setValue(self.rx_gain)
        self.tx_gain_slider.setValue(self.tx_gain)
        self.thresh_slider.setValue(int(self.threshold))
        self.targets_slider.setValue(self.num_targets)
        self.freq_input.setText(str(int(self.center_freq)))
        self.samp_input.setText(str(int(self.samp_rate)))
        self.template_combo.setCurrentText(self.template)
        self.adapt_cb.setChecked(self.adaptive_bw)
        self.sab_cb.setChecked(self.preamble_sabotage)
        self.pull_input.setText(str(self.clock_pull))
        self.stutter_cb.setChecked(self.stutter_enabled)
        self.stutter_clean_input.setText(str(self.stutter_clean))
        self.stutter_burst_input.setText(str(self.stutter_burst))
    ...
    def on_record_toggle(self, checked):
        ...
        if checked:
            ts = int(time.time())
            self.lock()
            self.file_sink.open(f"analysis_{ts}.sigmf-data")
            self.connect(self.hw_source, self.file_sink)
            self.unlock()
            self.record_btn.setText("LOGGING...")
            self.record_btn.setStyleSheet(danger_button())
        else:
            self.lock()
            self.disconnect(self.hw_source, self.file_sink)
            self.file_sink.close()
            self.unlock()
            self.record_btn.setText("LOG SIGMF")
...
if __name__ == "__main__":
    ...
    tb = PredatorJammer()
    signal.signal(signal.SIGINT, lambda sig, frame: tb.stop_all() or sys.exit(0))
    tb.start()
    tb.show()
    app.aboutToQuit.connect(lambda: tb.stop_all())
    sys.exit(app.exec_())
