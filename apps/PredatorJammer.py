...
class PredatorJammer(gr.top_block, Qt.QWidget):
    def __init__(self):
        ...
        # Tuning Panel in Header
        tuning_box = Qt.QGroupBox("Master Radio Controls")
        ...
        self.apply_btn = Qt.QPushButton("RESTART FLOWGRAPH")
        self.apply_btn.clicked.connect(self.restart_flowgraph)
        self.apply_btn.setStyleSheet("background-color: #004; color: white; font-weight: bold; padding: 5px;")
        ...
        # --- 2. MIDDLE SPLIT (Controls | Waterfall | Log) ---
        ...
        # Discovery
        hw_disc_box = Qt.QGroupBox("Device Discovery")
        ...
        hw_disc_grid.addWidget(Qt.QLabel("USRP Serial:"), 0, 0); hw_disc_grid.addWidget(self.serial_combo, 0, 1)
        self.scan_btn = Qt.QPushButton("SCAN DEVICES"); self.scan_btn.clicked.connect(self.on_scan_clicked); hw_disc_grid.addWidget(self.scan_btn, 1, 0)
        ...
        # RF Controls
        rf_box = Qt.QGroupBox("Gain & Output")
        ...
        self.fire_btn = Qt.QPushButton("DISABLE TX"); self.fire_btn.setCheckable(True); self.fire_btn.setStyleSheet("background-color: #050; color: white; font-weight: bold;"); self.fire_btn.toggled.connect(self.on_fire_toggle); rf_grid.addRow(self.fire_btn)
        hw_layout.addWidget(rf_box)
        
        # Presets
        pre_box = Qt.QGroupBox("Session Manager")
        ...
        self.save_btn = Qt.QPushButton("Save"); self.save_btn.clicked.connect(self.save_current_preset); pre_grid.addWidget(self.save_btn, 1, 0)
        ...
        # Tab 2: Interdiction Modes
        jam_tab = Qt.QWidget(); jam_layout = Qt.QVBoxLayout(jam_tab)
        self.tabs.addTab(jam_tab, "Interdiction")
        
        # Target Mode
        mode_box = Qt.QGroupBox("Targeting Mode")
        mode_layout = Qt.QVBoxLayout(mode_box)
        self.auto_radio = Qt.QRadioButton("Automatic Peak Tracking"); self.auto_radio.setChecked(True); self.auto_radio.toggled.connect(self.on_mode_change); mode_layout.addWidget(self.auto_radio)
        ...
        jam_layout.addWidget(mode_box)
        
        # Hydra / Surgical
        hydra_box = Qt.QGroupBox("Hydra Auto-Surgical Comb")
        hydra_grid = Qt.QFormLayout(hydra_box)
        self.hydra_cb = Qt.QCheckBox("Enable Hydra Detector"); self.hydra_cb.toggled.connect(self.on_hydra_toggle); hydra_grid.addRow(self.hydra_cb)
        ...
        jam_layout.addWidget(hydra_box)
        
        # Sticky Denial
        sticky_box = Qt.QGroupBox("Persistent Channel Denial")
        sticky_grid = Qt.QFormLayout(sticky_box)
        self.sticky_cb = Qt.QCheckBox("Enable Sticky Trap"); self.sticky_cb.toggled.connect(self.on_sticky_toggle); sticky_grid.addRow(self.sticky_cb)
        ...
        jam_layout.addWidget(self.manual_box); jam_layout.addStretch()

        # Tab 3: Protocol Manipulation
        prot_tab = Qt.QWidget(); prot_layout = Qt.QVBoxLayout(prot_tab)
        self.tabs.addTab(prot_tab, "Protocol")
        
        # Advanced Manipulations
        adv_box = Qt.QGroupBox("Stability & Preamble")
        adv_grid = Qt.QFormLayout(adv_box)
        self.adapt_cb = Qt.QCheckBox("Adaptive Bandwidth Sculpting"); self.adapt_cb.toggled.connect(self.on_adapt_toggle); adv_grid.addRow(self.adapt_cb)
        ...
        self.sab_input = Qt.QLineEdit(str(self.sabotage_duration)); self.sab_input.editingFinished.connect(self.on_sab_duration_change); adv_grid.addRow("Sabotage (ms):", self.sab_input)
        ...
        self.stutter_clean_input = Qt.QLineEdit(str(self.stutter_clean)); self.stutter_clean_input.editingFinished.connect(self.on_stutter_clean_change); adv_grid.addRow("Clean Frames:", self.stutter_clean_input)
        self.stutter_burst_input = Qt.QLineEdit(str(self.stutter_burst)); self.stutter_burst_input.editingFinished.connect(self.on_stutter_burst_change); adv_grid.addRow("Burst Frames:", self.stutter_burst_input)
    ...
    def on_pull_input_change(self):
        try: self.clock_pull = float(self.pull_input.text()); 
    ...
    def on_sab_duration_change(self):
        try: self.sabotage_duration = float(self.sab_input.text()); 
    ...
    def check_detections(self):
        if not self.hardware_connected: self.status_label.setText("OFFLINE"); self.status_label.setStyleSheet("font-size: 18px; font-weight: bold; background: #222; color: #555; border: 2px solid #333; border-radius: 5px;"); return
    ...
    def update_dynamic_params(self):
        ...
        for p in wf_def['params']:
            ...
            if p['type'] == 'entry':
                w = Qt.QLineEdit("0"); w.editingFinished.connect(lambda n=p['name'], widget=w: self.on_dynamic_change(n, widget.text())); self.param_layout.addRow(p['title'], w)
            elif p['type'] == 'options':
    ...
    def on_freq_change(self):
        try: self.center_freq = float(self.freq_input.text()); 
