...
int main(int argc, char* argv[]) {
    ...
    if (do_stream) {
        ...
        for (auto c : channels) {
            ...
            if (device->setFrequency(SOAPY_SDR_TX, c, freq) != 0) {
                std::cerr << "!!! Tuning Failed on Channel " << c << std::endl;
            }
            ...
            if (!ants.empty()) {
                std::cout << "[TUNE] Channel " << c << " Antenna: " << ants[0] << std::endl;
                device->setAntenna(SOAPY_SDR_TX, c, ants[0]);
            }
        }
        ...
        while (true) {
            ...
            while (sent < total_aligned) {
                ...
                if (ret == num_elems) {
                    ...
                    std::cerr << "DMA Write Error: " << ret << std::endl;
                    goto end_stream;
                }
            }
        }
