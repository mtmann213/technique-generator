...
public:
    ...
    static std::vector<std::complex<float>> correlatorConfusion(
        ...
        const std::string& confusion_mode = "both",
        float target_value = 1.0,
        const std::string& normalization_type = "peak",
        const std::string& filter_type = "none"
    );
    ...
    static std::vector<std::complex<float>> narrowbandNoise(
        ...
        double sample_rate_hz,
        double technique_length_seconds,
        const std::string& interference_type = "complex",
        float target_value = 1.0,
        const std::string& normalization_type = "peak",
        const std::string& filter_type = "none"
    );
    ...
    static std::vector<std::complex<float>> sweptNoise(
        ...
        double sample_rate_hz,
        double technique_length_seconds,
        const std::string& sweep_type = "sawtooth",
    ...
    static std::vector<std::complex<float>> phaseShiftedNoise(
        double bandwidth_hz,
        double sample_rate_hz,
        double technique_length_seconds,
        double phase_shift_deg = 180.0,
        double shift_rate_hz = 1000.0,
        float target_value = 1.0,
        const std::string& normalization_type = "peak",
        const std::string& filter_type = "none"
    );
...
private:
    ...
    static void normalizeSignal(std::vector<std::complex<float>>& samples, float target_value, const std::string& normalization_type);
    static void applySpectralShaping(std::vector<std::complex<float>>& samples, double bandwidth_hz, double sample_rate_hz, const std::string& filter_type, double rolloff = 0.35);
};
