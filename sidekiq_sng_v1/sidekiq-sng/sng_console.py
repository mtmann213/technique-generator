...
class SidekiqSngConsole:
    ...
    def main_menu(self):
        while True:
            ...
            print("-" * 60)
            print(
                f" [R] RADIO SETTINGS (Freq:{self.settings['freq']}, Rate:{self.settings['rate']}, Chan:{self.settings['chan']})"
            )
            ...
            if choice == '1':
                ...
                self.execute(stream=True)
            elif choice == 'g':
                self.execute(stream=False)
            elif choice == 'q':
                break

            if choice in ['1', '2', '3', 's', 'g']:
    ...
    def update_tech_params(self):
        ...
        params = []
        if t == "fhss":
            params = [("hops", "-200k 0 200k"), ("hop-dur", "0.01")]
        elif t == "comb":
            params = [("spacing", "30000"), ("spikes", "10")]
        elif t == "phase-noise":
            params = [("shift", "180"), ("shift-rate", "1000")]
        elif t == "fm-cosine":
            params = [("mod-rate", "1000")]
        elif t == "chunked-noise":
            params = [("sweep-rate", "1000"), ("spikes", "10")]
        elif t == "rrc":
            params = [("rolloff", "0.35")]
        elif t == "confusion":
            params = [("pulse-gap", "10"), ("mode", "both")]
        elif t == "noise-tones":
            params = [("hops", "-1M 0 1M")]
    ...
    def execute(self, stream=False):
        ...
        for k, v in self.settings.items():
            cmd.extend([f"--{k}", v])
        for k, v in self.tech_params.items():
            cmd.extend([f"--{k}", v])
        if stream:
            cmd.append("--stream")
        self.run_cmd(cmd)
